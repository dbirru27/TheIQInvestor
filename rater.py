...
class BreakoutRater:
    ...
    def _score_from_data(self, ticker, hist, info, db_conn=None, stock_yf=None):
        ...
        technical_score = min(100, round(sum(r.points for r in results if r.category in ["Momentum", "Breakout", "Timing"]) / 58 * 100))
        growth_score    = min(100, round(sum(r.points for r in results if r.category == "Growth")  / 38 * 100))
        quality_score   = min(100, round(sum(r.points for r in results if r.category == "Quality") / 18 * 100))
        context_score   = min(100, round(max(0, sum(r.points for r in results if r.category == "Context")) / 10 * 100))
